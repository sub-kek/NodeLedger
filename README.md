...
The current format is documented in [NODELEDGER.md](NODELEDGER.md). This README describes the Java API and intended
usage.
...
dependencies {
  implementation("space.subkek:NodeLedger:VERSION")
}
...
<dependency>
  ...
  <artifactId>NodeLedger</artifactId>
...
Do not instantiate documents, nodes, registry objects, or internal values directly. Use `NodeLedger.create()`,
`NodeLedger.open(path)`, and `NodeLedger.parser()`.
...
When saving back to the same opened path, NodeLedger appends new data and a new root record. Old bytes remain in the
file until trim.
...
After `document.trim(compacted)`, the document is associated with `compacted`, marked clean, and its `fileSize()` /
`lastTrimTimestamp()` metadata are updated.
...
Use `document.get(...)`, `document.set(...)`, and `document.remove(...)` for values. `NodeLedgerNode` is intended for
navigation and key inspection.
...
Invalid API arguments such as bad paths, unknown Java value types, or duplicate codec registration throw
